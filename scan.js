...
function renderBadges(badgesObj, username, validUntil) {
  ...
  if (certDate && validUntil) {
    const enchantedUntil = new Date(validUntil);
    const today = new Date();
    const effectiveDate = enchantedUntil < today ? enchantedUntil : today;
  ...
  activeBadges.forEach(b => {
    ...
    const enchantedUntil = validUntil ? new Date(validUntil) : null;
    const today = new Date();
    ...
    el.innerHTML = `
      ...
      <div class="scan-badge-content">
        ...
        <div class="scan-badge-field single-line">
          ...
          <span class="scan-badge-value">${formatDate(effectiveDate.toISOString())}</span>
        </div>
      </div>
    `;
    grid.appendChild(el);
