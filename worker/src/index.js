...
function buildMagicLinkEmailHtml(payload) {
  return `
    <div style="margin:0;padding:40px 16px;background:
      radial-gradient(circle at top left, rgba(104,166,255,.18), transparent 32%),
      radial-gradient(circle at top right, rgba(239,215,162,.14), transparent 28%),
      linear-gradient(180deg, #07101b 0%, #0a1221 100%);
      font-family:Inter,Arial,sans-serif;color:#eef2ff;">
      <div style="max-width:680px;margin:0 auto;border:1px solid rgba(255,255,255,.08);border-radius:24px;background:rgba(13,22,38,.9);box-shadow:0 26px 70px rgba(0,0,0,.38);overflow:hidden;">
        <div style="padding:30px 30px 18px;border-bottom:1px solid rgba(255,255,255,.07);text-align:center;">
          <div style="letter-spacing:.28em;text-transform:uppercase;font-size:12px;color:#efd7a2;margin-bottom:12px;">Mythical Helper Guild</div>
          <div style="font-family:Georgia,serif;font-size:32px;line-height:1.08;color:#efd7a2;font-weight:700;">Return To Your Record</div>
          <div style="margin-top:10px;font-size:15px;line-height:1.55;color:rgba(238,242,255,.84);">
            A trusted access link has been prepared for the guild record kept under this email.
          </div>
        </div>
        <div style="padding:28px 30px 30px;">
          <div style="margin:0 0 18px;font-size:16px;line-height:1.7;color:#eef2ff;">
            Open the link below to re-enter the record, revise its details, and prepare the certificate again when the moment is right.
          </div>
          <div style="margin:0 0 24px;padding:18px 20px;border:1px solid rgba(239,215,162,.18);border-radius:18px;background:rgba(239,215,162,.06);font-size:14px;line-height:1.7;color:#d9e3f5;">
            Some gifts, traditions, and quiet works of wonder are carried out with human help. This record exists so that care, effort, and entrusted responsibility may one day be seen clearly.
          </div>
          <div style="text-align:center;margin:0 0 24px;">
            <a href="${payload.magicLink}" style="display:inline-block;padding:14px 24px;border-radius:999px;background:#5c72c9;color:#eef2ff;text-decoration:none;font-weight:700;letter-spacing:.04em;">
              Open The Record
            </a>
          </div>
          <div style="margin:0 0 10px;font-size:12px;letter-spacing:.14em;text-transform:uppercase;color:rgba(239,215,162,.76);">
            Direct Link
          </div>
          <div style="padding:14px 16px;border-radius:14px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);font-size:13px;line-height:1.65;color:rgba(238,242,255,.74);word-break:break-all;">
            ${payload.magicLink}
          </div>
          <div style="margin-top:18px;font-size:12px;line-height:1.6;color:rgba(238,242,255,.52);text-align:center;">
...
function buildMagicLinkEmailText(payload) {
  return [
    ...
    "",
    "A trusted access link was prepared for the guild record kept under this email.",
...
async function sendZohoMessage(env, accessToken, accountId, payload) {
  ...
  const message = {
    ...
    ccAddress: fromAddress,
    subject: "Your MythicalHelper return link",
