...
// 这个 Worker 代码需要部署到 Cloudflare Workers
// 用于解决 IPv6-only 服务器无法访问 Stripe API 的问题

/*
Cloudflare Worker 代码 (部署到 Cloudflare Workers):
...
2. 进入 Workers & Pages
3. 创建新 Worker 命名为 "stripe-worker"
4. 将上面的代码复制到 Worker 编辑器
5. 在 Worker 设置中添加环境变量 STRIPE_SECRET_KEY
6. 部署 Worker
7. 在 Cloudflare DNS 中添加 CNAME 记录：stripe-worker -> <worker-subdomain>.workers.dev
*/
